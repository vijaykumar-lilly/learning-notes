...
export default function Definition({ term, children, variant = 'default', example }: DefinitionProps) {
  ...
  return (
    <>
      <div className={cn(
        'my-6 p-4 sm:p-6 rounded-xl border-l-4 relative group',
        variant === 'important'
        ...
        <button
          ...
          className={cn(
            "absolute top-2 right-2 p-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity",
      ...
      <ZoomModal isOpen={isZoomed} onClose={() => setIsZoomed(false)} title={term}>
        {content}
      </ZoomModal>
    </>
  )
